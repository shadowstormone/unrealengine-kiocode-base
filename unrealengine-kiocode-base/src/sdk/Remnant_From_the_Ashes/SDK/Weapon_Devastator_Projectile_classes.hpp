#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: Weapon_Devastator_Projectile

#include "Basic.hpp"

#include "Engine_structs.hpp"
#include "BP_Projectile_Stick_Sphere_Base_classes.hpp"


namespace SDK
{

// BlueprintGeneratedClass Weapon_Devastator_Projectile.Weapon_Devastator_Projectile_C
// 0x0028 (0x04F0 - 0x04C8)
class AWeapon_Devastator_Projectile_C final : public ABP_Projectile_Stick_Sphere_Base_C
{
public:
	struct FPointerToUberGraphFrame               UberGraphFrame_Weapon_Devastator_Projectile_C;     // 0x04C8(0x0008)(ZeroConstructor, Transient, DuplicateTransient)
	class UTweenComponent*                        Tween;                                             // 0x04D0(0x0008)(BlueprintVisible, ZeroConstructor, InstancedReference, IsPlainOldData, NonTransactional, NoDestructor, HasGetValueTypeHash)
	class UAudioComponent*                        Weapon_Devastator_Primary_Fire_Bolt_Projectile_Start_SC; // 0x04D8(0x0008)(BlueprintVisible, ZeroConstructor, InstancedReference, IsPlainOldData, NonTransactional, NoDestructor, HasGetValueTypeHash)
	TArray<class AActor*>                         UniqueHits;                                        // 0x04E0(0x0010)(Edit, BlueprintVisible, ZeroConstructor, DisableEditOnTemplate, DisableEditOnInstance)

public:
	void ExecuteUbergraph_Weapon_Devastator_Projectile(int32 EntryPoint);
	void ReceiveBeginPlay();
	bool IsSprayHit(const struct FHitResult& Hit);

public:
	static class UClass* StaticClass()
	{
		BP_STATIC_CLASS_IMPL("Weapon_Devastator_Projectile_C")
	}
	static const class FName& StaticName()
	{
		STATIC_NAME_IMPL(L"Weapon_Devastator_Projectile_C")
	}
	static class AWeapon_Devastator_Projectile_C* GetDefaultObj()
	{
		return GetDefaultObjImpl<AWeapon_Devastator_Projectile_C>();
	}
};
DUMPER7_ASSERTS_AWeapon_Devastator_Projectile_C;

}

