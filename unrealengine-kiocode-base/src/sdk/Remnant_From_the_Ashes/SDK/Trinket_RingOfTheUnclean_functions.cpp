#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: Trinket_RingOfTheUnclean

#include "Basic.hpp"

#include "Trinket_RingOfTheUnclean_classes.hpp"
#include "Trinket_RingOfTheUnclean_parameters.hpp"


namespace SDK
{

// Function Trinket_RingOfTheUnclean.Trinket_RingOfTheUnclean_C.ExecuteUbergraph_Trinket_RingOfTheUnclean
// (Final, UbergraphFunction)
// Parameters:
// int32                                   EntryPoint                                             (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void ATrinket_RingOfTheUnclean_C::ExecuteUbergraph_Trinket_RingOfTheUnclean(int32 EntryPoint)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("Trinket_RingOfTheUnclean_C", "ExecuteUbergraph_Trinket_RingOfTheUnclean");

	Params::Trinket_RingOfTheUnclean_C_ExecuteUbergraph_Trinket_RingOfTheUnclean Parms{};

	Parms.EntryPoint = EntryPoint;

	UObject::ProcessEvent(Func, &Parms);
}


// Function Trinket_RingOfTheUnclean.Trinket_RingOfTheUnclean_C.OnUnequipped
// (Event, Public, BlueprintEvent)

void ATrinket_RingOfTheUnclean_C::OnUnequipped()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("Trinket_RingOfTheUnclean_C", "OnUnequipped");

	UObject::ProcessEvent(Func, nullptr);
}


// Function Trinket_RingOfTheUnclean.Trinket_RingOfTheUnclean_C.OnEquipped
// (Event, Public, BlueprintEvent)

void ATrinket_RingOfTheUnclean_C::OnEquipped()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("Trinket_RingOfTheUnclean_C", "OnEquipped");

	UObject::ProcessEvent(Func, nullptr);
}


// Function Trinket_RingOfTheUnclean.Trinket_RingOfTheUnclean_C.OnPostComputeStats
// (Event, Protected, BlueprintEvent)

void ATrinket_RingOfTheUnclean_C::OnPostComputeStats()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("Trinket_RingOfTheUnclean_C", "OnPostComputeStats");

	UObject::ProcessEvent(Func, nullptr);
}


// Function Trinket_RingOfTheUnclean.Trinket_RingOfTheUnclean_C.ModifyInspectInfo
// (Event, Public, HasOutParams, HasDefaults, BlueprintCallable, BlueprintEvent)
// Parameters:
// class AActor*                           Actor                                                  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// class UItemInstanceData*                InInstanceData                                         (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, InstancedReference, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// struct FInspectInfo&                    Info                                                   (BlueprintVisible, BlueprintReadOnly, Parm, OutParm, ReferenceParm)
// bool*                                   Rtn                                                    (Parm, OutParm, ZeroConstructor, IsPlainOldData, NoDestructor)

void ATrinket_RingOfTheUnclean_C::ModifyInspectInfo(class AActor* Actor, class UItemInstanceData* InInstanceData, struct FInspectInfo& Info, bool* Rtn)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("Trinket_RingOfTheUnclean_C", "ModifyInspectInfo");

	Params::Trinket_RingOfTheUnclean_C_ModifyInspectInfo Parms{};

	Parms.Actor = Actor;
	Parms.InInstanceData = InInstanceData;
	Parms.Info = std::move(Info);

	UObject::ProcessEvent(Func, &Parms);

	Info = std::move(Parms.Info);

	if (Rtn != nullptr)
		*Rtn = Parms.Rtn;
}

}

